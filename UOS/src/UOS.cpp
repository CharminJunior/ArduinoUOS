...
void My_eerom::W(String name, String Text) {
  if(R(name) != Text) {
    D(name);
...
String My_eerom::R(String nane) {
  ...
  while (addr < EEPROM.length()) {
    ...
    // ถ้าชื่อไม่ตรงกับ nane ให้ข้ามข้อมูล (ค่าข้อมูล) ไปจนเจอ '&' ตัวที่สอง
    if (Lite != nane) {
      // ข้ามข้อมูลจนเจอ '&'
      while (addr < EEPROM.length()) {
      ...
      continue; // ไปอ่านชุดถัดไป
    }

    //J1++;

    // ชื่อตรงกับ nane → อ่านค่าข้อมูล (จนเจอ '&')
    Vode = "";
    while (addr < EEPROM.length()) {
      char c = (char)EEPROM.read(addr);
      if (c == '&') {
    ...
    return Vode; // คืนค่าข้อมูลที่เจอ
  }

  H1 = -1;
  return "-1"; // ไม่เจอชื่อใน EEPROM
}

String My_eerom::R(String nane, int &rawLength) {
  long addr = 0;
  String Lite = "";
  String Vode = "";
  rawLength = 0;
  ...
  while (addr < EEPROM.length()) {
    ...
    // คำนวณความยาวข้อมูลดิบ ตั้งแต่ตำแหน่ง @ ถึง & ตัวที่สอง (addr คือหลังข้าม & ตัวที่สองแล้ว)
